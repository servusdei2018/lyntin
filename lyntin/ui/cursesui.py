...
# copyright (c) Free Software Foundation 2003-2020
#
...
# Our dictionary has the structure:
# { <keyboard-sequence>:
...
def startup_hook(args):
    """
  Loads the command dictionary. This should be done not earlier than the
  "startup_hook".
  """
    modules.modutils.load_commands(
        {
            "curses.bind": (curses_bind_cmd, "key= responce= quiet:boolean=false"),
            "curses.unbind": (curses_unbind_cmd, "key quiet:boolean=false"),
        }
    )
...
def bindings_persist(args):
    """
  Persistance function to save current bindings with #write
  """
    quiet = args["quiet"]
...
def is_a_char(ch):
    """
  Determine if ch is a character. Needed because curses.ascii is not
  locale-aware
  """
        ...
        for lineattrs in self.lines_[self.startline_ : self.startline_ + self.h_]:
            ...
            for (line, attr) in lineattrs:
                ...
                while current_len >= rest:
                    self.window_.addnstr(
                        current_y, current_x, line[offset:], rest, attr
                    )
                    current_x = 0
                    current_y += 1
                    if current_y >= self.h_:
...
class inputbox:
    ...
    def attach(self, window):
        self.window_ = window
        window.keypad(1)
        window.timeout(10)  # keyboard polling timeout in milliseconds
    ...
    def do_command(self, ch):
        ...
        if (ch & ~0xFF) == 0 and (ascii.isprint(ch) or chr(ch) in string.letters):
            self.string_ = (
                self.string_[: self.curx_] + chr(ch) + self.string_[self.curx_ :]
            )
            self.curx_ += 1
            self._align(1)
            self._reset()
            ...
            if self.curx_ > 0:
                self.curx_ -= 1
                self.string_ = (
                    self.string_[0 : self.curx_] + self.string_[self.curx_ + 1 :]
                )
                self._align(1)
            self._reset()
        elif ch == ascii.HT:
            ...
            if not self.history_:
                self.history_ = filter(
                    lambda x: x.find(self.string_) != -1, exported.get_history(1000)
                )
            if self.history_:
  ...
  """
        ...
        for single in line.splitlines(1):
            ...
            for tok in tokens:
                if ansi.is_color_token(tok):
                    currentcolor, leftover = ansi.figure_color(
                        [tok], currentcolor, leftover
                    )
                elif tok:
                    attr = default_attr
            ...
            if current:
                ...
                if not lasttok.endswith("\n"):  # it's a prompt
                    # append newline to prompts coming from other sessions
                    if ses != exported.get_current_session():
                        current.append(("\n", curses.A_NORMAL))
                    else:
                        if self.cfg_compact_ and self.prompt_ == current:
                            # remove an identical prompt from previous output buffer
    ...
    """
        ...
        if ses != None and ses != exported.get_current_session():
            ...
            pretext = [("[", attr), (ses.getName(), attr | curses.A_BOLD), ("] ", attr)]
    ...
    def runui(self):
        """
    Loop for user input polling and for mud output
    """
        global HELP_TEXT
            ...
            while self.running_:
                ...
                if not out:
                    ...
                    if not scrollback:
                        ...
                        wborder.noutrefresh()
                        out = scroller(
                            curses.newwin(out_h, screen_w, scroll_h + 1, 0), lines
                        )
                    out.redraw()
                ...
                if keyboard_buffer and not hotkey_buffer:
                    ...
                    if ch == curses.ERR:
                        ...
                        try:
                            (i, o, x) = select.select(
                                select_input_list, [], [], select_timeout
                            )
                        except:
                            # it's probably a stray EINTR
                        ...
                        if not i:
                            ...
                            if ch == curses.ERR:
                                timestamp_now = time()
                                if ((timestamp_now - timestamp) > 0.2) or (
                                    dirty_count > self.cfg_lazy_
                                ):
                                    out.redraw(self.cfg_maxscrollback_)
                                    select_timeout = 100
                                    dirty_count = 0
                    ...
                    if ch < 256:
                        ...
                        if binding:
                            ...
                            lambda x: x.startswith(hotkey_buffer), bindings.keys()
                        ):
                            hotkey_buffer = ""
                        continue
                    else:
                ...
                if ch == curses.KEY_PPAGE:
                    if not scrollback:
                        # create scrollback window at next iteration
                        scrollback = 1
                        out = None
                    else:
                ...
                elif ch in (curses.KEY_RESIZE, curses.ascii.FF):
                    # force screen redraw
                    out = None
                    continue
