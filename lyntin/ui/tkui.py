...
# copyright (c) Free Software Foundation 1999-2020
#
...
# list of foreground color codes and what color they map to in RGB.
fg_color_codes = {
    "30": "#000000",
    "31": "#aa0000",
    "32": "#00dd00",
...
# list of background color codes and what color they map to in RGB.
bg_color_codes = {
    "40": "#000000",
    "41": "#ff0000",
    "42": "#00ff00",
...
# specified a color yet. when using DEFAULT make sure you clone it first
...
class Tkui(base.BaseUI):
  ...
  """
    ...
    def __init__(self):
        ...
        self._txt = ScrolledText(
            self._topframe, fg="white", bg="black", font=fnt, height=20
        )
        self._txt.pack(side="bottom", fill="both", expand=1)

        self._txt.bind("<KeyPress>", self._ignoreThis)
        self._txtbuffer = ScrolledText(
            self._topframe, fg="white", bg="black", font=fnt, height=20
        )
    ...
    def _ignoreThis(self, tkevent):
        ...
        # make sure that ctrl-c doesn't get caught, so that windows can copy
        # the buffer
        ...
        self._entry.focus()
        if tkevent.char:
            # pass events we don't want to deal with to the entry widget
    ...
    def write_internal(self, args):
        ...
        color, leftover = buffer_write(
            mess, self._txt, self._currcolors, self._unfinishedcolor
        )
...
class CommandEntry(Entry):
  ...
  """
    ...
    """
        ...
        self.bind("<KeyPress-F1>", self.callBinding)  # for help
    ...
    """
        (text, position) = self._partk.get_completion(self.get(), self.index("insert"))
        self.delete(0, "end")
        self.insert(0, text)
...
class NamedWindow:
  ...
  """
    ...
    """
        ...
        color, leftover = buffer_write(
            msg, self._txt, self._currcolors, self._unfinishedcolor
        )
...
class Autotyper:
  ...
  """
    ...
    """
        ...
        self._txt = ScrolledText(
            self._frame, fg="white", bg="black", font=fnt, height=20
        )
        self._txt.pack(side=TOP, fill=BOTH, expand=1)
  ...
  """
    ...
    if msg.type == message.ERROR:
        ...
        if myui._do_i_echo == 1:
            ...
            # if echo is not on then we don't print this
    ...
    # go through and handle writing all the data
    index = 0
    start = 0

    # prepend the session name to the text if this is not the current
    # session sending text, and if the Message is session scoped.
    ...
    # each session has a saved current color for MUDDATA. we grab color--
    # or use our default if we don't have one for this session.
    # some sessions have an unfinished color --in case we got a part of an
    # ansi  color code in one message, and the other part in another.
    if msg.type == message.MUDDATA:
