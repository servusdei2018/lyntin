...
class Textui(base.BaseUI):
  ...
  """
    ...
    """
        ...
        # grab stdin stream, enabling its redirection by another module
    ...
    def runui(self):
        ...
        # termios allows us to change echo for a terminal.
        # we import it if possible.
        try:
            import termios
        except ImportError:
            self._tio = 0
        ...
        if config.options.has_key("readline"):
            try:
                ...
                # grab the readlinerc file if they have one so the user can set
                # some readline oriented things
        ...
        if self._tio == 0 or self._rline == 1:
            exported.write_error(
                "Warming: echo off is unavailable.  " + "Your password will be visible."
            )
        ...
        try:
            while not self.shutdownflag:
                ...
                if data != None:
                    ...
                    # FIXME - we need to know we're ending _before_ we block for
                    # the next input, otherwise we will shut down except
    ...
    """
        ...
        # if this isn't the current session, prepend the session name to
        # the output
        pretext = ""
        ...
        # each session has a saved current color for mud data. get
        # that current color--or user our default if we don't have one yet.
        if self._currcolors.has_key(ses):
            color = self._currcolors[ses]
        else:
            color = list(DEFAULT_COLOR)

        # some sessions have an unfinished color
