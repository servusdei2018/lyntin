...
class Engine:
  ...
  """
    ...
    def __init__(self):
        ...
        # event queue that holds all the events in the system.
        self._event_queue = Queue.Queue()

        # a lock for writing stuff to the ui--makes sure that multiple
        # things don't write to the ui simultaneously
        self._ui_lock = thread.allocate_lock()

        # master shutdown flag for event queue handling.
        self._shutdownflag = 0

        # Lyntin counts the total number of errors it has encountered.
        # we can stop if it gets too high.
        self._errorcount = 0

        # listeners exist at an engine level. if you create an input hook,
        # you get the input hook for ALL sessions.
        self._listeners = {}
    ...
    """
        ...
        c.add(
            ...
            config.BoolConfig(
                "repeathistory",
                utils.convert_boolean(cops.get("repeathistory", 1)),
                0,
        ...
        c.add(
            ...
            config.CharConfig(
                "commandchar",
                config.options.get("commandchar", "#"),
                0,
        ...
        c.add(
            ...
            config.BoolConfig(
                "debugmode",
                utils.convert_boolean(cops.get("debugmode", 0)),
                0,
        ...
        c.add(
            ...
            config.BoolConfig(
                "promptdetection",
                utils.convert_boolean(cops.get("promptdetection", 0)),
                0,
        ...
        c.add(
            ...
            config.BoolConfig(
                "ansicolor",
                utils.convert_boolean(cops.get("ansicolor", 1)),
                1,
        ...
        c.add(
            ...
            config.BoolConfig(
                "mudecho",
                utils.convert_boolean(cops.get("mudecho", 1)),
                0,
    ...
    """
        ...
        for mem in self._hooks.keys():
            data.append(
                "   %s - %d registered functions" % (mem, self._hooks[mem].count())
            )
    ...
    """
        # clean up list of threads
        self._threadCleanup()

        # create and initialize a new thread and add it to our list
        t = Thread(None, func)
        t.setDaemon(1)
        t.setName(name)
        t.start()
        ...
        while not ev.isSet():
            try:
                ...
                if tout > 0:
                    ...
                    # we are too late. drop 10 ticks
                    wakeup_time += 10
                    continue
                event.SpamEvent(
                    hookname="timer_hook", argmap={"tick": self._current_tick}
                ).enqueue()
    ...
    """
        ...
        for mem in inputlist:
            ...
            if len(mem) > 0 and mem.startswith(commandchar):
                ...
                # get the first token without the commandchar
                ses = mem.split(" ", 1)[0][1:]
                ...
                if self._sessions.has_key(ses):
                    ...
                    if len(input) < 2:
                        ...
                        self.handleUserData(
                            input[1], internal=1, session=self._sessions[ses]
                        )
            ...
            # it is not a valid !-expression. give it to the default session
            historyitems.append(mem)

            # no command char, so we pass it on to the session.handleUserData
            session.handleUserData(mem, internal)

        # don't record internal stuff or input that isn't to be echoed
    ...
    """
        if name == "":
            ...
            # ensure that the common session is the last one we would select
        ...
        # if a name is specified, switch to that session
    ...
    """
        ...
        exported.hook_spam(
            "session_change_hook", {"new": newsession, "previous": previous_session}
        )
    ...
    """
        ...
        exported.write_error(
            "WARNING: Unhandled error encountered (%d out of %d)."
            % (self._errorcount, 20)
        )
    ...
    @rtype: list of strings
    """
        # get status from session.getStatus()
        data = ses.getStatus()

        # get our managers' stati
    ...
    try:
        ...
        for mem in optlist:
            if mem[0] == "--help":
                ...
                # ini files OVERRIDE the default options. multiple ini files may
                # be specified. each new ini file will OVERRIDE the contents of
                # the previous ini file where the two files intersect
      ...
      """
        ...
        if config.options["datadir"]:
            ...
            if os.path.exists(lyntinrcfile):
                # read .lyntinrc first, so that new files can overwrite the
                # contents therein
