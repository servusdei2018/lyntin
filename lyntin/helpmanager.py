...
class HelpManager(manager.Manager):
  ...
  """
    ...
    """
        ...
        if found == 0 and fqn != "":
            # find all instances of categorylist[0] in help tree
            potentialroots = []
            ...
            # walk through all of the nodes named categorylist[0] and see if
            # they hold categorylist[1:]
            ...
            # as though that was what was entered. Otherwise return an error.
            if len(foundnodes) == 1:
                ...
                error = (
                    "Could not find exact match for '%s'.  We did find these matches:"
                    % fqn
                )
        ...
        if type(tree) == types.DictType:
            ...
            catlist.sort()
            if tree.has_key("__doc__"):
                if "__doc__" in toplist:
                    toplist.remove("__doc__")
                if "__doc__" in catlist:
                    catlist.remove("__doc__")
    ...
    """
        ...
        if len(keys) > 0:
            categories = keys[:-1]
            name = keys[-1]
            if len(name) > 0 and name[0] == self._engine.getConfigManager().get(
                "commandchar"
            ):
                name = name[1:]
