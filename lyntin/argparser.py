...
class ArgumentParser:
  ...
  """
    ...
    """
        ...
        # we display it whenever we have a ParserException from user input
        self.syntaxline = ""
    ...
    """
        ...
        for i in range(0, len(parsedspec)):
            ...
            if argname.find(":") > -1:
                ...
                # extra argname assignment for consistency
                argname, typespec = argname, "string"

            if len(argname) >= 1 and argname.endswith("*"):
                if argdef != None:
                    raise ParserException, "cannot specify a default value for a collection argument (%s=%s)" % (
                        argname,
                        argdef,
                    )
                ...
                if len(argname) >= 2 and argname.endswith("**"):
                    ...
                    if i < len(parsedspec) - 1:
                        raise ParserException, "named collection argument must be the last argument (%s)" % (
                            argname
                        )
            ...
            if defaultSeen and not parser.defaultset:
                raise ParserException, "Argument without default value (%s) seen after default values already specified" % (
                    argname
                )
            ...
            if not namedCollector and not indexCollector:
                ...
                if self.parsers.has_key(argname):
                    raise ParserException, "Multiple argument named %s specified." % (
                        argname
                    )
        ...
        for i in range(0, len(arguments)):
            ...
            if val == None:
                if foundNamedArg and not self.extraindexparser:
                    raise ParserException, "Non-named argument (%s) found after Named argument" % (
                        key
                    )
                ...
                if self.parsers.has_key(key):
                    ...
                    matchedkeys = [
                        arg for arg in self.parsers.keys() if arg.find(key) == 0
                    ]
                    ...
                    if len(matchedkeys) == 1:
                        ...
                        if len(matchedkeys) == 0:
                            raise ParserException(
                                "Invalid named argument: %s=%s" % (key, val)
                            )
                        else:
                            raise ParserException(
                                "Ambiguous named argument: %s=%s %s"
                                % (key, val, matchedkeys)
                            )

                parser.parseInto(key, val, argdict)

        # check that everything has been specified, putting in defaults
        # where available
        for key in self.parsers.keys():
            if not argdict.has_key(key):
                # be careful here with the extra defaultset value since
                ...
                if not defaultset and not self.getOption("noparsing"):
                    raise ParserException, "Must specify a value for argument %s" % (
                        key
                    )
                else:
                    argdict[key] = default
  ...
  """
    ...
    """
        if argdict.has_key(self.argname):
            raise ParserException, "Multiple values for argument %s given" % (
                self.argname
            )
        else:
...
class StringChecker(TypeChecker):
  ...
  """
    ...
    def __init__(self, typename, typeargs):
        if typeargs:
            raise ParserException, "TypeArgs (%s) specified for non-configurable type (%s)" % (
                typeargs,
                typename,
            )
        return
...
class ChoiceChecker(TypeChecker):
  ...
  """
    ...
    """
        if not typeargs:
            raise ParserException(
                "TypeArgs (%s) not specified for %s type - must allow at least one choice."
                % (typeargs, typename)
            )
    ...
    """
        ...
        if len(possibilities) == 0 or len(possibilities) > 1:
            raise ParserException, "Invalid argument, must be one of %s." % (
                self._choices,
            )
        else:
