...
TIMESPAN_REGEXP = re.compile(
    r"^(?P<days>\d+d)?(?P<hours>\d+h)?(?P<minutes>\d+m)?(?P<seconds>\d+s?)?$"
)
TIME_REGEXP1 = re.compile(r"^(?P<hour>[1-9]|1[0-2])(?P<ampm>a|p)$")
TIME_REGEXP2 = re.compile(
    r"^(?P<hour>[1-9]|1[0-2]):(?P<minute>[0-5][0-9])(:(?P<second>[0-5]\d))?(?P<ampm>a|p)?$"
)
TIME_REGEXP3 = re.compile(
    r"^(?P<hour>0|1[3-9]|2[0-3]):(?P<minute>[0-5][0-9])(:(?P<second>[0-5]\d))?$"
)
...
# handle regular expression syntax and flags
REG_REGEXP = re.compile("^(r\[.*?)(\$?\][Ii]*)$")

# to find variables in the subject
...
def compile_regexp(text, anchors=0, stars=0):
  ...
  """
    ...
    if REG_REGEXP.match(text) != None:
        # then we should compile this as a regular expression
...
def expand_text(filter, fulllist):
  ...
  """
    ...
    # there's wildcards
        ...
        # it with .*
        # FIXME - this needs to account for escaped
        # * stuff
...
def split_commands(splitchar, text):
  ...
  """
    ...
    while matchob:
        (b, e) = matchob.span()
        # count braces. if the entire segment doesn't have a full set of
        # matched braces, we treat this semi-colon as a split point.
        left = 0
        right = 0
...
def _find_next_break(token, marker, wrapcount, wraplength):
  ...
  @rtype: int
  """
    # check to see to see if we need to find a break
    ...
    # no carriage returns. handle spaces.
...
def wrap_text(textlist, wraplength=50, indent=0, firstline=0):
  ...
  """
    ...
    for i in range(0, len(textlist)):
        # gloss over color tokens
        if ansi.is_color_token(textlist[i]):
            continue

        # factor text tokens into word wrapping
        marker = 0

        # handles offset for not indenting the first line
        if firstline:
        ...
        while x != -1:
            # insert carriage return, any indent, and lstrip the line
            if textlist[i][x] == "\n":
                if indent:
                    textlist[i] = (
                        textlist[i][: x + 1]
    ...
    # the next line joins the list with no separator
    if firstline:
...
def build_graph(numbers_dict):
  ...
  """
    ...
    graph = []
    graph.append(
        "  " + " ".ljust(maxlength + 8) + "0" + ("-" * (max / divisor)) + ("%d" % max)
    )
...
def parse_timespan(timespan):
  ...
  """
    ...
    if (
        not timespec["days"]
        and not timespec["hours"]
        and not timespec["minutes"]
        and not timespec["seconds"]
    ):
        raise ValueError("Invalid timespan string.")
...
def parse_time(timearg):
  ...
  """
    match = (
        TIME_REGEXP1.match(timearg)
        or TIME_REGEXP2.match(timearg)
        or TIME_REGEXP3.match(timearg)
    )
...
def escape(s):
  ...
  """
    ...
    # FIXME - this won't work if people use regexps that involve $ in the
    # middle somewhere.  we should build a regexp to do the substitution
    # which handles various situations.
...
def expand_vars(text, varmap):
  ...
  """
    ...
    # sort them in order of lengthiest first
    varmapkeys.sort(lambda x, y: cmp(len(y), len(x)))
    i = 0

    # iterate through the text and expand them one by one
    while i < len(text):
        mem = text[i]
        if i != 0:
            memm1 = text[i - 1]
        ...
        if (mem == "%" or mem == "$") and memm1 != "\\":
            ...
            if ccount == 1 and j < len(text):
                ...
                # we handle the closures
                if text[j] == "{":
                    closure = text.find("}", j)
                    # if there's no closure, then we set it to the end of the text
                    if closure == -1:
                        closure = len(text) - 1
...
def expand_placement_vars(input, expansion):
  ...
  """
    ...
    if len(vars) > 0:
        ...
        # find what all variables translate to
        for mem in vars:
            if mem.find(":") < 0:
                start = int(mem)
                if start == -1:
