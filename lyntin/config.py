...
# a list of all the modules Lyntin has dynamically imported
# or have been imported via the #import command.
lyntinmodules = []

# application options--these are adjusted by command-line
# arguments only
options = {
    "datadir": "",
    "moduledir": [],
...
class ConfigBase:
  ...
  """
    ...
    """
        ...
        # otherwise it returns the adjusted value (string -> what we're
        # supposed to store)
...
class ConfigManager(manager.Manager):
  ...
  """
    ...
    def __init__(self, e):
        # map of session -> (map of config names -> items)
        self._config = {}
        self._engine = e
    ...
    def addSession(self, newsession, basesession):
        # if we have no basesession, we can't do anything
