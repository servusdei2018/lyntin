...
class SubstituteData:
    ...
    """
        ...
        listing = [
            "substitute {%s} {%s}" % (mem, utils.escape(self._substitutes[mem]))
            for mem in listing
        ]
    ...
    def getSubstituteInfoMapping(self):
        ...
        for mem in self._substitutes.keys():
            l.append(
                {"item": mem, "substitution": utils.escape(self._substitutes[mem])}
            )
        return l
...
class SubstituteManager(manager.Manager):
    ...
    def getParameters(self, item):
        if item == "substitute":
            return [
                ("item", "The thing to substitute."),
                ("substitution", "The thing to substitute with."),
            ]

        if item == "antisubstitute":
            return [
                ("item", "The thing whose presence denotes we shouldn't substitute.")
            ]
...
def load():
    ...
    for mem in exported.get_active_sessions():
        tc = config.BoolConfig(
            "ignoresubs", 0, 1, "Allows you to turn on and turn off substitutions."
        )
