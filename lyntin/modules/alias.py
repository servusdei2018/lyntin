...
class AliasManager(manager.Manager):
  ...
  """
    ...
    def getParameters(self, item):
        ...
        return [("alias", "The alias name."), ("expansion", "The alias expansion.")]
    ...
    user_filter_hook.
    """
        # check for aliases here. if we find some, we do the variable
        # expansion and then recurse over the result
        ses = args["session"]
...
def alias_cmd(ses, args, input):
  ...
  """
    ...
    # they typed '#alias' or '#alias x' so print the relevant aliases
    if not command:
