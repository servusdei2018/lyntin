  ...
  """
    ...
    def __repr__(self):
        if self._repeat == 0:
            return truncate(
                "%s [%s] %d {%s}"
                % (self._id, self._ses._name, self._offset, self._cmd),
                60,
            )
        return truncate(
            "%s [%s] %d(r) {%s}" % (self._id, self._ses._name, self._offset, self._cmd),
            60,
        )
    ...
    def __repr__(self):
        return truncate(
            ...
            % (
                self._id,
                self._ses._name,
                time.strftime("%d %b %H:%M:%S", time.localtime(self._offset)),
                self._cmd,
            ),
            60,
        )
...
class Scheduler:
  ...
  """
    ...
    """
        ...
        # execute for this second and any previous seconds that were missed
        sec = int(time.time())
        ...
        # execute all the events we've found
        for mem in events:
            if not mem._quiet:
...
commands_dict["schedule"] = (
    schedule_cmd,
    "tick= event= repeat:boolean=false quiet:boolean=false",
)
...
# this is a dict holding the DEFAULT_TICKER values.  we copy this
# when we don't have them.
DEFAULT_TICKER = {"len": 3, "warn_len": 2, "enabled": 0}
...
def _addtickevents(ses):
  ...
  """
    ...
    sevent = SchedEvent(
        ses._ticker["len"], ses, _tickfunc, repeat=1, quiet=1, tag=tick_tagname
    )
    sevent._args = [ses]
    ...
    # and add it to the schedule
    warnsevent = SchedEvent(
        ses._ticker["len"], ses, _tickwarnfunc, repeat=1, quiet=1, tag=tickwarn_tagname
    )
...
def tickon_cmd(ses, args, input):
  ...
  """
    ...
    # check to make sure there isn't already a tick event for this session
...
def tickoff_cmd(ses, args, input):
  ...
  """
    ...
    exported.write_message("tickoff: session %s ticker disabled." % ses.getName(), ses)
  ...
  """
    ...
    if size == 0:
        exported.write_message(
            "ticksize: ticksize is %d seconds." % ses._ticker["len"], ses
        )
        return
...
def tickwarnsize_cmd(ses, args, input):
  ...
  """
    ...
    if size == 0:
        exported.write_message(
            "tickwarnsize: tickwarnsize is %d seconds." % ses._ticker["warn_len"], ses
        )
        return
    ...
    if size > ses._ticker["len"]:
        exported.write_error(
            ...
            + "to ticksize.\nCurrent ticksize is %s." % ses._ticker["len"],
            ses,
        )
        return
    ...
    exported.write_message("tickwarnsize: tickwarn length set to %s." % str(size), ses)
