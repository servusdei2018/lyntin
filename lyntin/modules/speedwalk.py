...
class SpeedwalkManager(manager.Manager):
    ...
    """
        ...
        if (
            not self._hashes.has_key(ses)
            or exported.get_config("speedwalk", ses) == 0
            or verbatim == 1
        ):
            return text
        ...
        if (
            not sdata._dirs
            or not sdata._regexp
            or text in sdata._excludes
            or text in sdata._aliases
            or not sdata._regexp.match(text)
        ):
            return text
...
def load():
    ...
    for mem in exported.get_active_sessions():
        tc = config.BoolConfig(
            "speedwalk", 1, 1, "Allows you to turn on and turn off speedwalk handling."
        )
