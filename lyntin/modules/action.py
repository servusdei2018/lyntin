...
# placement variable regular expression
...
class ActionData:
    ...
    """
        ...
        if not actionlist:
            actionlist = filter(
                lambda x: not self._disabled.has_key(x[6]), self._actions.values()
            )
        ...
        # go through all the lines in the data and see if we have matches
        for (
            action,
            actioncompiled,
            response,
            ...
            if match:
                ...
                # fill in response variables from those that matched on the
                # trigger
    ...
    """
        ...
        return [
            "%s tag={%s}" % ((" enabled", "disabled")[disabled], mem)
...
class ActionManager(manager.Manager):
    ...
    def getParameters(self, item):
        ...
        return [
            ("trigger", "Text that triggers the action."),
...
def action_cmd(ses, args, input):
  ...
  """
    ...
    # they typed '#action' so print out all the current actions
    if not action:
    ...
    try:
        ...
        if not quiet:
            exported.write_message(
                ...
                % (trigger, action, color, priority, str(tag)),
                ses,
            )
    except:
...
def load():
    ...
    for mem in exported.get_active_sessions():
        # need a separate BoolConfig for each session
        tc = config.BoolConfig(
            "ignoreactions", 0, 1, "Allows you to turn off action handling."
        )
