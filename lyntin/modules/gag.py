...
class GagManager(manager.Manager):
    ...
    def getParameters(self, item):
        if item == "gag":
            return [
                ("text", "The text whose presence indicates we should gag the line.")
            ]

        if item == "antigag":
            return [
                ("item", "The item whose presence indicates we shouldn't gag the line.")
            ]
    ...
    """
        ...
        if exported.get_config("ignoresubs", ses, 0) == 0 and self._gagdata.has_key(
            ses
        ):
