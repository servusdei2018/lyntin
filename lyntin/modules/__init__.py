  ...
  """
    if module.__file__ != name + "c" and module.__file__ != name:
        exported.write_error(
            "possible name conflict: '%s' and '%s'" % (name, module.__file__)
        )
...
def load_modules():
  ...
  """
    ...
    for mem in _module_list:
        # kip over all files that start with a _. this allows hackers to be
        # working on a module and not have it die every time.
    ...
    if moduledirlist:
        for moduledir in moduledirlist:
            ...
            # put the contents of the directory in our _module_list
            for mem in _module_list:
                mem2 = get_module_name(mem)
                if mem2.startswith("_"):
                    continue
