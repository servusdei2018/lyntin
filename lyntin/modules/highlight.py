...
class HighlightData:
    ...
    """
        if text:
            ...
            for mem in hlist:
                ...
                for m in miter:
                    # loop for multiple instances of the highlight
                    begin, end = m.span()
                    hl = self._highlights[mem][1]
                    textlist = self.highlight(textlist, begin, end - begin, hl)

            # sweep through the text string to update our current color and
            # leftover color attributes
            self._currcolor, self._colorleftover = ansi.figure_color(
                textlist, self._currcolor, self._colorleftover
            )
    ...
    @rtype: list of strings
    """
        # find the place to put the highlight
        i = 0
        ...
        # if the string to highlight begins and ends in the same token
    ...
    def getInfoMappings(self):
        ...
        for mem in self._highlights.keys():
            l.append({"style": self._highlights[mem][0], "text": utils.escape(mem)})
...
class HighlightManager(manager.Manager):
    ...
    def getParameters(self, item):
        ...
        return [
            ("style", "The formatting style to highlight the text with."),
