...
def config_cmd(ses, args, input):
  ...
  """
    ...
    # configuration stuff in general and for this session
    if not name:
    ...
    if not ci:
        exported.write_error(
            "config: config manager does not recognize %s as a config item." % name
        )
        return

    if not value:
        # print out everything we know about this config item.
        output = "config: %s\n\ncurrent value: %s\n\n%s\n" % (
            name,
            ci.toString(),
            utils.wrap_text(ci._description, wraplength=60),
        )
...
def diagnostics_cmd(ses, args, input):
  ...
  """
    ...
    message.append(
        "   lyntin: %s" % (constants.VERSION[: constants.VERSION.find("\n")])
    )
    ...
    exported.write_message(
        "This information can be dumped to a "
    ...
    if logfile:
        ...
        try:
            ...
            exported.write_error(
                "diagnostics: Error writing to file %s. %s" % (logfile, e)
            )
