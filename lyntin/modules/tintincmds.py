...
def if_cmd(ses, args, input):
  ...
  category: commands
  note: original if_cmd code contributed by Sebastian John
  """
...
def read_cmd(ses, args, input):
  ...
  category: commands
  note: http reading contributed by Sebastian John
  """
    ...
    if not contents[0].startswith(c):
        exported.lyntin_command(
            "%sconfig commandchar %s" % (c, contents[0][0]), internal=1, session=ses
        )
