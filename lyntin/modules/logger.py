...
class LoggerData:
    def __init__(self, session):
        ...
        self._strip_ansi = 0  # whether or not to strip ansi--0 is off, 1 is on
        self._user_input = []  # pending user inputs:
        self._prompt = None  # pending mud prompt:
        self._userprefix = ""
        ...
        try:
            ...
            if not input.endswith("\n"):
                ...
                if self._prompt:
                    ...
                    # don't log prompts. wait for user input or mud output
                    self._prompt = (input,)
            else:
                # ordinal output from mud
                if self._prompt:
                    # prompt pending, so log it first
                    self.log(self._prompt[0] + "\n")
                    self._prompt = None
                self.log(input)
        finally:
    ...
    """
        try:
            ...
            if self._prompt:
                ...
                # something wrong, the server doesn't send prompts. dump all the
                # input we have into log now.
                self._log_user_input(self._userprefix)
        finally:
            self._lock.release()
    ...
    try:
        ...
        if databuffer:
            ...
            exported.write_message(
                "log: dumped %d lines of databuffer to logfile" % buffer.count("\n"),
                ses,
            )
        ...
        exported.write_message(
            "log: starting logging to '%s'%s." % (logfile, stripansimessage), ses
        )
    except Exception, e:
        exported.write_error(
            "log: logfile cannot be opened for appending. %s" % (e), ses
        )


commands_dict["log"] = (
    log_cmd,
    "logfile= databuffer:boolean=false stripansi:boolean=true userprefix=",
)
