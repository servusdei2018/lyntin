...
# reverse lookup for easier debugging
...
# strings to help with the telnet control code negotiation
DD = DO + DONT
WW = WILL + WONT
DDWW = DD + WW
...
class SocketCommunicator:
  ...
  """
    ...
    def __init__(self, e, ses, host, port):
        ...
        # prompt regex that used to split the incoming text
        self._prompt_regex = self._buildPromptRegex()

        # the regex that used to split the incoming text
        delimiters = (IAC + GA, IAC + TELOPT_EOR, "\n")
        # a regexp that matches any of the above delimiters
        self._line_regex = re.compile(
            "(" + "|".join(delimiters) + ")", re.MULTILINE | re.DOTALL
        )
        ...
        # log all the telnet stuff we're doing here for debugging
        self._controllog = []
    ...
    """
        if not self._sock:
            ...
            exported.hook_spam(
                "connect_hook", {"session": ses, "host": host, "port": port}
            )
        else:
    ...
    """
        readers, e, w = select.select([self._sock], [], [], 0.2)
        if readers:
    ...
    """
        # this catches BOTH '' and None
        if not data:
            return data

        spamargs = {"session": self._session, "data": data, "dataadj": data}
        spamargs = exported.hook_spam(
            "net_read_data_filter", argmap=spamargs, mappingfunc=exported.filter_mapper
        )
    ...
    """
        ...
        try:
            ...
            while not self._shutdownflag:
                ...
                if newdata:
                    ...
                    # walk through each line of data, adjusting last_index to the
                    # end of the previous match
                    for m in self._line_regex.finditer(alldata):
                        oneline = alldata[last_index : m.end()]
                        last_index = m.end()
                        self.handleData(oneline)
                    # keep remainder (empty if alldata ended with a delimiter)
                    data = alldata[last_index:]

                elif newdata == "":
                    # if we got back an empty string, something's wrong so dump it
                    if data:
                    ...
                    # Then we have rest of the input which is neither delimited
                    # prompt nor complete line. We haven't seem this server
                    # delimiting it's prompts with telnet GA or EOR option.
                    # Handle these data because the socket read was timed out.
                    self.handleData(data)
                    data = ""
        ...
        # if we get to here, shutdown the socket
        try:
        ...
        # sometimes a mud will send echo off, so we keep it on
        self._config.change("mudecho", "on")

        # tell user of disconnect
        event.OutputEvent(
            message.Message("Lost connection to: %s\n" % self._host)
        ).enqueue()
    ...
    """
        ...
        if self._shutdownflag == 0:
            ...
            # enables things like compressors and data transformers
            spamargs = {"session": self._session, "data": data, "dataadj": data}
            spamargs = exported.hook_spam(
    ...
    """
        ...
        for i in range(count):
            event.SpamEvent(
                hookname="bell_hook", argmap={"session": self._session}
            ).enqueue()
        ...
        if not self._config.get("promptdetection") or data.endswith("\n"):
            ...
            event.SpamEvent(
                hookname="prompt_hook",
                argmap={"session": self._session, "prompt": data},
            ).enqueue()
            ...
            if data[i + 1] == NOP:
                ...
                if data[i + 1] in DDWW:
                    ...
                    if option[2] == ECHO:
                        ...
                        # return the args (if no one handled it) or None (if someone
                        # handled it and raised a StopSpammingException).
                ...
                # handles SB...SE stuff
                    ...
                    if option[2] == TERMTYPE and option[3] == SEND:
                        self.write(
                            IAC + SB + TERMTYPE + IS + self._termtype + IAC + SE, 0
                        )
                        self.logControl(
                            "send: IAC SB TERMTYPE IS " + self._termtype + " IAC SE"
                        )
                    else:
                        args = {"session": self._session, "data": option}
                        # return the args (if no one handled it) or None (if someone
                ...
                # in case they passed us something odd, remove the IAC and go on
                else:
                    data = data[:i] + data[i + 1 :]
