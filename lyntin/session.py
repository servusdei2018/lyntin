  ...
  """
    ...
    def setupCommonSession(self):
        ...
        tc = config.BoolConfig(
            "snoop",
            utils.convert_boolean(config.options["snoopdefault"]),
            0,
    ...
    """
        ...
        if self.getName() != "common":
            ...
            exported.hook_spam(
                "disconnect_hook",
                {"session": self, "host": self._host, "port": self._port},
            )
    ...
    """
        for line in message.strip().split("\n"):
            exported.hook_spam(
                "to_mud_hook", {"session": self, "data": line, "tag": tag}
            )
        ...
        if self._socket:
            ...
            # if we don't have a socket then we can't send anything
    ...
    """
        # recursion point.  everything is registered as a filter and
        # recurses accordingly.
        spamargs = {
            "session": self,
            "internal": internal,
        ...
        # no more recursion, so it's safe to unescape things
        input = input.replace("\\;", ";")
        input = input.replace("\\$", "$")
        input = input.replace("\\%", "%")

        # send regular data to the mud
    ...
    @type  input: string
    """
        # handles ansi color codes that get broken  idd-transmission when
        # mud data is chunked and sent across the network.
        ...
        # add the new input to the databuffer
        self.addToDataBuffer(input)

        # split input into a series of lines
